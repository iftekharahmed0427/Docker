...
#
#   1. On first boot, copy the baked Wine prefix TEMPLATE into the server volume,
#      so the prefix is owned by this server's own uid and Wine accepts it.
#   2. Keep the config's port + player cap in sync with the panel.
#   3. Start a headless X display.
#   4. Exec the game in the FOREGROUND as the main process, so it owns the
#      panel's stdin/stdout and receives SIGINT directly for a clean save.
